<x-adminLay>
    ...
    <div
        ...
        <a href="/mahasiswa/create" class="block h-full py-2 text-sm md:text-base">Mahasiswa Baru<i
    ...
    @if($mhs->isEmpty())
        @if($query)
            <p class="py-2 text-center">Data mahasiswwa tidak ditemukan</p>
        @else
            <p class="py-2 text-center">Belum ada data mahasiswa</p>
        @endif
    @else
                ...
                <td class="px-1 py-2 text-xs border">
                    <div>
                        ...
                        <form action="{{ route('mahasiswa.destroy', $m->nim) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="underline text-rose-600 hover:text-rose-700" onclick="return confirm('Yakin ingin menghapus data mahasiswa?')">hapus</button>
                        </form>
                    </div>
                </td>
                @endauth
