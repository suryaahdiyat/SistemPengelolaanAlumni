<x-adminLay>
	...
	<div
		...
		<a href="/alumni/create" class="block h-full py-2 text-sm md:text-base">Alumni Baru<i
				class='mx-2 bx bx-plus-circle'></i></a>
	</div>
	@endauth
	@if($alumni->isEmpty())
	@if($query)
	<p class="py-2 text-center">Belum ada data alumni</p>
	@else
	<p class="py-2 text-center">Belum ada data alumni</p>
	@endif
	@else
	<table class="w-full my-2 overflow-hidden text-center bg-white border-2 rounded-lg shadow-lg table-auto">
		<thead>
			<tr>
				...
				<th class="px-1 py-2 text-xs border">IPK</th>
				<th class="px-1 py-2 text-xs border">Lulus Dari</th>
				@auth
		...
		<tbody>
			...
			<tr>
				...
				<td class="px-1 py-2 text-xs border">{{ $a->mahasiswa->nama }}</td>
				<td class="px-1 py-2 text-xs border">{{ $a->mahasiswa->nim }}</td>
				<td class="px-1 py-2 text-xs border">{{ $a->ipk }}</td>
				<td class="px-1 py-2 text-xs border">{{ $a->tanggal_lulus }}</td>
				@auth	
				<td class="px-1 py-2 text-xs border">
					<div>
						<a href="/alumni/{{ $a->id }}/edit"
							class="underline duration-150 text-cyan-500 hover:text-cyan-700">ubah</a>
						<form action="/alumni/{{ $a->id }}" method="POST">
							@csrf
							@method('DELETE')
							<button class="underline text-rose-600 hover:text-rose-700" onclick="return confirm('Yakin ingin menghapus data alumni?')">hapus</button>
						</form>
					</div>
				</td>
				@endauth
